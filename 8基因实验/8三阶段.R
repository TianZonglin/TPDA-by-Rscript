...
##############################################################
Sort1_Or_NotSort2 <- TRUE    #是否运行了关联规则算法排序
            LSdim <- 0        #离散值 { 使 LSdim<-0 为 不离散 }
         weight_1 <- 0.001     #互信息阀值
         weight_2 <- 0.001    #互信息阀值
         weight_3 <- 0.001     #互信息阀值
##############################################################

z<-data.frame()
gene<-data.frame()
if(Sort1_Or_NotSort2==TRUE){
  z<-read.csv("D:\\RST\\8基因实验\\express100_afterF.csv",header = F,stringsAsFactors = FALSE)
  gene<-z
}else{
  z<-read.csv("D:\\RST\\8基因实验\\Gene8.csv", header=FALSE)
  gene<-z
}
row.names(z)<-z[,1]
z<-z[-1]
