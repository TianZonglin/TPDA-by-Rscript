...
ruleBack<-read.csv("D:\\RST\\8基因实验\\Gene8.csv",header = F)
...
#读入关联规则的结果
hc <- read.csv("D:\\RST\\8基因实验\\sum_data.csv",header = T,stringsAsFactors = F)
hc <- hc[,-2] 
hc <- hc[,-3] 
hc <- hc[,-3]   
...
write.csv(gen,"D:\\RST\\8基因实验\\genNode.csv",row.names = F)
...
sx <- read.csv("D:\\RST\\8基因实验\\genNode.csv",header = T)
...
two <- one[order(one[,2],decreasing=T),] #顺序【sheng序 100...1 】
two <- two[,-2]

write.csv(two,"D:\\RST\\8基因实验\\express100_afterT.csv",row.names = F)
